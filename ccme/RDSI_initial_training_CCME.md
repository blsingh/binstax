# PowerShell 5.1 Mastery Syllabus — Claude/CCME Edition

This guide is adapted for a persistent CCME workspace driven by Claude Code. Each learning turn is submitted through `input.md`, answered in `output.md`, and summarized durably in `memory.md`. It focuses on shifting from a procedural (mouse-click) mindset to an algorithmic (CLI-driven) workflow using **Windows PowerShell 5.1**. Fill in bracketed information `[...]` before committing the relevant learning turn to `input.md`.
...
### One Dedicated CCME Project

Use one CCME project directory for the entire syllabus:

```text
projects/RDSI_initial/
├── input.md
├── output.md
├── memory.md
├── RDSI_initial_training_CCME.md
├── scripts/
├── test-data/
└── notes/
```

Treat this directory as the persistent equivalent of one long learning thread:

- `RDSI_initial_training_CCME.md` is the authoritative curriculum and must not be edited by the agent unless explicitly requested.
- `input.md` contains only the current learning turn. A commit is the submission signal.
- `output.md` receives the response, appended newest-last.
- `memory.md` records durable progress: current phase, demonstrated concepts, misconceptions corrected, exercises completed, and the exact next step.
- `scripts/`, `test-data/`, and `notes/` hold work products created during the syllabus.
- Do not manually edit `output.md`.
- Do not advance phases merely because content was discussed. Advance only after the learner can explain and apply it.

### CCME Turn Discipline

For every committed learning turn, Claude should:

1. Read `memory.md` first.
2. Read only the relevant syllabus phase unless broader context is required.
3. Continue from the exact current learning state.
4. Ask one focused guiding question at a time unless the task is explicitly a review or synthesis.
5. Avoid generating a finished solution before the learner has attempted the reasoning.
6. Append the response to `output.md`.
7. Update `memory.md` only with durable learning state, not a transcript.
8. Avoid files outside `projects/RDSI_initial/`.

### Standing Preamble for Claude
...
## How Each Phase Runs in CCME

At the beginning of a phase, commit a turn that names the phase and asks Claude to read that phase from this syllabus. During the phase, keep each `input.md` submission narrow: answer Claude's prior question, show your attempted reasoning or code, and request the next step. At the end of the phase, require a mastery check and a memory update before moving forward.

The syllabus prompts describe Claude's teaching behavior. They are not invitations for Claude to complete the exercises on the learner's behalf.
